...
def replay_models(
    ...
    *,
    ...
    model_a, schema_a = _load_version(session, model_version_a)
    model_b, schema_b = _load_version(session, model_version_b)
    ...
    windows, window_end_ts = _build_windows(epochs, settings.window_size)
    preds_a, conf_a = _predict(model_a, windows)
    preds_b, conf_b = _predict(model_b, windows)
...
def _build_windows(
    ...
    windows = build_windows(epochs, window_size=window_size, allow_padding=False)
    tensors = [window.tensor for window in windows]
    end_ts = [window.end_ts for window in windows]
    return tensors, end_ts


def _load_version(session: Session, version: str) -> tuple[Any, Any]:
...
def _predict(model: Any, windows: list[np.ndarray]) -> tuple[list[str], list[float]]:
    if not windows:
        return [], []
...
def _load_ground_truth(
    ...
    if not window_end_ts:
        return None
    rows: list[tuple[datetime, str | None]] = session.execute(
...
def _build_summary(
    ...
    summary = {
        ...
        "confidence_delta": {
            "mean": float(np.mean(conf_a) - np.mean(conf_b))
    ...
    if ground_truth:
        labels = sorted(
            set([label for label in ground_truth if label])
            | set(preds_a)
            | set(preds_b)
        )
